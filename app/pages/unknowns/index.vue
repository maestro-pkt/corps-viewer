  ...
  <div class="container mx-auto px-4">
    <div class="flex flex-col md:flex-row gap-4">
      ...
      <div class="md:w-1/3 bg-gray-100 p-4 rounded-lg">
        ...
        <Card class="p-2">
          <template #content>
            ...
            <DataTable :value="unks" v-model:selection="selectedFile" v-model:filters="filters"
              :globalFilterFields="['name', 'years']" filterDisplay="row" @rowClick="selectFile" selectionMode="single"
              sortMode="single" sortField="name" dataKey="key" scrollable scrollHeight="400px"
              :virtualScrollerOptions="{ itemSize: 46 }" tableStyle="min-width: 15rem">
              <template #header>
      ...
      <div class="md:w-1/3 bg-gray-100 p-4 rounded-lg">
        ...
        <p> VidID:{{ vidId }}</p>
        <p>Type: {{ vidType }}</p>
        <video-player :options="videoOptions" :vidId="vidId" :vidType="vidType" endpoint="/api/unknowns" />
      </div>
      ...
      <p> Or another idea is to convert the unknown index page into a three column layout and
        ...
        <p>Add a permanent ignore so the file stops showing up in unknown list</p>
