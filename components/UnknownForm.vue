...
const onFormSubmit = (e) => {
  ...
  $fetch(`/api/unknowns/update`, {
    method: "post",
    body: {
      unkId: props.record.key,
      corpsId: selectedCorpsId.value,
      year: selectedYear.value,
      dateAdded: selectedDateAdded.value,
      path: props.record.path,
      title: '',
    },
  });

  console.log('remove from unknowns list...  or force a refresh');
...
fileinfo ??
  ...
  <div class="card flex justify-center">
    <Form :initialValues :resolver @submit="onFormSubmit" class="flex flex-col gap-4 w-full sm:w-80">
      <FormField v-slot="$field" name="corpsId" initialValue="" :resolver="zodCorpsIdResolver"
        class="flex flex-col gap-1">
        <FloatLabel variant="on">

          <Select id="corpsId" filter v-model="selectedCorpsId" :options="corpsList.default" optionLabel="name"
            optionValue="corpsId" placeholder="Corps" class="w-full md:w-56">
      ...
      <FormField v-slot="$field" name="year" initialValue="" :resolver="zodYearResolver" class="flex flex-col gap-1">
        <FloatLabel variant="on">
          <Select id="corpsYear" v-model="selectedYear" :options="yearList" class="w-full md:w-56" />
      ...
      <FormField v-slot="$field" name="dateAdded" initialValue="" class="flex flex-col gap-1">
        <FloatLabel variant="on">
          <DatePicker id="dateAdded" name="dateAdded" fluid v-model="selectedDateAdded" />
          <Message v-if="$field.dateAdded?.invalid" severity="error" size="small" variant="simple">{{
